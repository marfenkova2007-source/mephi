...
int main() {
    ...
    std::chrono::milliseconds multithreadDuration;

    {
        ...
        multithreadDuration = std::chrono::duration_cast<std::chrono::milliseconds>(duration);

        std::cout << threads.size() << " threads have run in " << multithreadDuration.count() << " ms" << std::endl;
        std::cout << "Total waited for mutex: " << std::chrono::duration_cast<std::chrono::milliseconds>(primes.GetTotalTimeWaitingForMutex()).count() << std::endl;
        std::cout << "Total time under mutex: " << std::chrono::duration_cast<std::chrono::milliseconds>(primes.GetTotalTimeUnderMutex()).count() << std::endl;
        CheckProperties(primes);
        assert(primes.GetTotalTimeUnderMutex() > 60ms);
    }

    {
        ...
        std::cout << "1 thread has run in " << std::chrono::duration_cast<std::chrono::milliseconds>(duration).count() << " ms" << std::endl;
        std::cout << "Total waited for mutex: " << std::chrono::duration_cast<std::chrono::milliseconds>(primes.GetTotalTimeWaitingForMutex()).count() << std::endl;
        std::cout << "Total time under mutex: " << std::chrono::duration_cast<std::chrono::milliseconds>(primes.GetTotalTimeUnderMutex()).count() << std::endl;
        CheckProperties(primes);
        assert(duration >= multithreadDuration * 1.3);
        assert(primes.GetTotalTimeWaitingForMutex() < 30ms);
    }
