...
public:
    PeerPiecesAvailability():bitfield_("") {};
    ...
    bool IsPieceAvailable(size_t pieceIndex) const{
        ...
        unsigned char currentByte = bitfield_[pieceIndex / 8];
        int pos = 7 - (pieceIndex%8);
        int bitValue = (currentByte>>pos) & 1;
    ...
    void SetPieceAvailability(size_t pieceIndex){
        size_t idx = pieceIndex/8;
        if (idx >= bitfield_.size()) {
            bitfield_.resize(idx+1, '\0');
        }
...
protected:
    ...
    std::atomic<bool> terminated_{false};  // флаг, необходимый для завершения цикла общения с пиром
